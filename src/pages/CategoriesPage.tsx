...
export function CategoriesPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApi(() => categoriesService.getAll(includeInactive), [includeInactive])
  ...
  const saveAction = useAsyncAction(async (values: CategoryFormValues) => {
    if (editing && editing !== 'new') {
      return categoriesService.update(editing.id, values)
    }
    return categoriesService.create(values)
  })

  const deleteAction = useAsyncAction((id: number) => categoriesService.remove(id))

  const toggleAction = useAsyncAction((category: Category) =>
  ...
  const active = categories?.filter((c) => c.isActive).length ?? 0
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage event categories and organize your events.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-bold text-gray-500">{inactive}</p>
        </div>
      </div>
      ...
      {!loading && !error && categories && categories.length === 0 && (
        <EmptyState message="No categories available." action={<Button onClick={() => setEditing('new')}>Create Category</Button>} />
      )}
                  ...
                  <tr key={category.id} className="transition hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <Button variant="secondary" onClick={() => setViewing(category)}>
                          View
                        </Button>
      ...
      {/* Create / Edit Modal */}
      <Modal open={editing !== null} title={editing === 'new' ? 'Create Category' : 'Edit Category'} onClose={() => setEditing(null)}>
        <CategoryForm
      ...
      {/* View Modal */}
      <Modal open={viewing !== null} title="Category Details" onClose={() => setViewing(null)}>
        <div className="space-y-4 text-sm">
          ...
          <div>
            ...
            <ActiveBadge isActive={viewing?.isActive ?? false} />
          </div>
        </div>
      </Modal>
